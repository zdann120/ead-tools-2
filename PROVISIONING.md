# How to provision a new environment for EuroAmericaTools
...
* Enter domains to secure (for example): `euroamerica.design`
* Enter webroot path: `/home/deployer/apps/EuroAmericaTools/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/euroamerica.design/fullchain.pem /etc/ssl/EuroAmericaTools.crt
ln -s /etc/letsencrypt/live/euroamerica.design/privkey.pem /etc/ssl/EuroAmericaTools.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/EuroAmericaTools/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
39 7,19 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/EuroAmericaTools/shared/public/
```
